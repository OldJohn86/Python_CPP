...
def read_ini(config, option):
    ...
    keys: List[Any] = cf.options(option)
...
class SSHConnection(object):
    ...
    def _connect(self):
        if self._transport is None:
            transport: Transport = paramiko.Transport(self._host, self._port)
            try:
...
def send_mail(config, msg1, msg2):
    ...
    mail_postfix: str = str(mail_info.get('postfix', None))
...
if __name__ == "__main__":
    current_path = sys.argv[0].rstrip('/kt_release_v3.py')
    # print(current_path)
