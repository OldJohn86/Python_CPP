    ...
    oam_uint8  *oui)
{
    oam_int32 len = 0;
    oam_pdu_hdr_t *pHdr;
    oam_uint8 null_mac[OAM_MACADDR_LEN] = {0, 0, 0, 0, 0, 0};
    ...
    oam_llid_mac_get_adapt(0, &pHdr->src_mac);
...
void handle_cs_oams(
    ...
    oam_uint32 len)
{
    oam_pdu_hdr_t   *oam_pdu = (oam_pdu_hdr_t *)frame;

    switch (oam_pdu->opcode) {
        case CS_OAM_OPCODE_HELLO:
#ifdef HAVE_ONU_HELLO
            onu_hello_proc(oam_pdu->data, (oam_int32)(len - sizeof(oam_pdu_hdr_t)),
                           g_oam_port);
#endif
            break;
...
#ifdef HAVE_ONU_EVENT
            onu_event_proc(oam_pdu->data, (oam_int32)(len - sizeof(oam_pdu_hdr_t)),
                           g_oam_port);
#endif
            break;
