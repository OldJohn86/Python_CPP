 ...
 */
static oam_boolean g_oam_send_flag = TRUE;
...
/* CHIP       : ----                                                         */
/*                                                                           */
void oam_set_send_flag(oam_boolean send_flag)
/*                                                                           */
...
/*****************************************************************************/
{
    ...
    oam_dump_pkt("send OAM", port, llid, buf, len);

    intf->frame_tx++;
    oam_pkt_send_adapt(port, llid, buf, len);
}
...
/*****************************************************************************/
{
    ...
    if (parm->mask & OAM_MUX_PAR_CTL_MASK_UNIDIR) {
        OAM_LOG_DEBUG("oam_mux_par_ctl_req: unidir %d\n",
                      parm->local_unidir);
    }

    if (parm->mask & OAM_MUX_PAR_CTL_MASK_LINK_STATUS) {
        OAM_LOG_DEBUG("oam_mux_par_ctl_req: link_status %d\n",
    ...
    if (set_action) {
        OAM_LOG_DEBUG("set action at port 0x%08x: mux %d par %d\n",
                      intf->llid, mux_action, par_action);
        oam_port_lpbk_set_adapt(intf->llid, mux_action, par_action);
    }
}
...
/*****************************************************************************/
{
    oam_send(intf->llid, OAM_PON_PORT_ID, pdu, len);
}
