...
static oam_boolean oam_len_check = OAM_FALSE;
/*
 * slow protocol address, refer 802.3ah
 */
static oam_uint8 oam_slow_protocols_multicast[OAM_MACADDR_LEN]
    = {0x01, 0x80, 0xc2, 0x00, 0x00, 0x02};

/*
 * Cortina OUI, the 3 bytes of cortina Mac address
 */
static oam_uint8 oam_oui_imst[OAM_OUI_LEN] = {0x00, 0x13, 0x25};
static oam_uint8 oam_oui_tk[OAM_OUI_LEN] = {0x00, 0x0d, 0xb6};

/*
 * Passive OUI
 */
static oam_uint8 oam_oui_pas[OAM_OUI_LEN] = {0x00, 0x0c, 0xd5};
...
/* CHIP       : ----                                                         */
/*                                                                           */
static oam_boolean oam_macaddr_higher(
    oam_uint8 *mac1,
    oam_uint8 *mac2)
/*                                                                           */
...
/* CHIP       : ----                                                         */
/*                                                                           */
static oam_int32 oam_proc_var_cont(
    oam_var_cont_t  *curr,
    oam_int32       *max,
    oam_var_cont_t  **next)
/*                                                                           */
...
/* CHIP       : ----                                                         */
/*                                                                           */
static oam_int32 oam_d2_proc_var_desc(
...
/* CHIP       : ----                                                         */
/*                                                                           */
oam_uint32 oam_d2_set_mib_variable(
    oam_if_t *intf,
    oam_uint8 branch,
    oam_uint8 leaf,
...
/*****************************************************************************/
{
    ...
    oam_pon_stats_get_adapt(FALSE, &stats);
    ...
    switch (branch) {
        case OAM_PASSAVE_BRANCH:
            switch (leaf) {
                ...
                case OAM_PASSAVE_OAM_TX:
                    IFMIBVARSET(intf->info_tx + intf->unique_event_tx + intf->dup_event_tx +
                                intf->lb_control_tx + intf->var_req_tx + intf->var_rsp_tx +
                                intf->org_specific_tx + intf->unsupported_tx);
                    break;

                case OAM_PASSAVE_OAM_RX:
                    IFMIBVARSET(intf->info_rx + intf->unique_event_rx + intf->dup_event_rx +
                                intf->lb_control_rx + intf->var_req_rx + intf->var_rsp_rx +
                                intf->org_specific_rx + intf->unsupported_rx);
                    break;
            ...
            OAM_LOG_DEBUG( "branch 0x%x leaf 0x%x, len %d\n",
        ...
        case OAM_INTF_COUNTERS_MIB_BRANCH:
            switch (leaf) {
                ...
                case OAM_INTF_LEAF_ETH_ADM_STATE:
                    var = onu_aal_get_admin_state(intf->llid);
                    IFMIBVARSET(var);
                    break;

                case OAM_INTF_LEAF_ETH_AUTONEG_ADM_STATE:
                    var = onu_aal_get_auto_neg_admin_state(intf->llid);
                    IFMIBVARSET(var);
                    break;

                case OAM_INTF_LEAF_ETH_PORT_CAP:
                    var = onu_aal_get_port_cap(intf->llid);
                    IFMIBVARSET(var);
                    break;

                case OAM_INTF_LEAF_ETH_AUTONEG_CAP:
                    var = onu_aal_get_port_auto_neg_cap(intf->llid);
                    IFMIBVARSET(var);
                    break;

                case OAM_INTF_LEAF_ETH_FEC_MODE:
                    var = onu_aal_get_port_fec_mode(intf->llid);
                    MIBVARSET(var);
                    break;
#endif
        ...
        case OAM_INTF_CONTROL_MIB_BRANCH:
            switch (leaf) {
#if 0 /* this value large than 0xFF */
...
/* CHIP       : ----                                                         */
/*                                                                           */
oam_uint32 oam_set_mib_variable(
    oam_if_t *intf,
    oam_uint8 branch,
    oam_int16 leaf,
    ...
    switch (branch) {
        case OAM_INTF_COUNTERS_MIB_BRANCH:
            switch (leaf) {
                ...
                case OAM_INTF_LEAF_PON_RX_ERR:
                    STDMIBVARSET(stats.pon_crcerr_cnt + stats.pon_crc8_err + stats.pon_err_symbol +
                                 stats.pon_sld_err);
                    break;
...
/* CHIP       : ----                                                         */
/*                                                                           */
static oam_int32 oam_proc_pdu_sanity(
    oam_if_t *intf,
    oam_pdu_hdr_t *pdu,
    oam_int32 len)
/*                                                                           */
...
/*****************************************************************************/
{
    ...
    oam_int16 flags = ntohs(pdu->flags);
    ...
    switch (pdu->code) {
        case OAM_PDU_CODE_INFO:
            if (!(flags & OAM_FLAG_LINK_FAULT)) {
                ...
                if ((!intf->d2_compatible &&
                    ...
                    if (!curr) {
                        OAM_LOG_DEBUG("Remote info TLV expected\n");
                        return -1;
                    }

                    if (oam_proc_tlv(curr, &max, &next)) {
                        OAM_LOG_DEBUG("Invalid remote info TLV\n");
                        return -1;
                    }

                    if (curr[0] != OAM_INFO_TLV_REMOTE_INFO) {
                        OAM_LOG_DEBUG("Remote info TLV missing\n");
                        return -1;
                    }
                ...
                while (curr) {
                    if (oam_proc_tlv(curr, &max, &next)) {
                        OAM_LOG_DEBUG("Invalid info TLV\n");
                        return -1;
                    }

                    if (curr[0] && (curr[0] != OAM_INFO_TLV_ORG_SPEC)) {
                        OAM_LOG_DEBUG("Unexpected TLV type\n");
                    }
        ...
        case OAM_PDU_CODE_EVENT:
            ...
            if (((oam_pdu_event_t *)pdu)->seq_num == intf->seq_num_last_rx) {
                OAM_LOG_DEBUG("Duplicated event\n");
                return -1;
            }
            ...
            while (curr) {
                ...
                if ((curr[0] != OAM_EVENT_TLV_ERR_SYM_PERIOD) &&
                    ...
                    curr[0]) {
                    OAM_LOG_DEBUG("Unknown TLV type\n");
                }
        ...
        case OAM_PDU_CODE_VAR_REQ:
            if (!(intf->remote_oam_config & OAM_INFO_CONFIG_OAM_MODE)) {
                OAM_LOG_DEBUG("Unexpected VarReq\n");
                return -1;
            }

            while (curr) {
                if (oam_proc_var_desc((oam_var_desc_t *)curr, &max, &next_var_desc)) {
                    OAM_LOG_DEBUG("Invalid variable descriptor\n");
                    return -1;
                }

                curr = (oam_uint8 *)next_var_desc;
            }
        ...
        case OAM_PDU_CODE_VAR_RSP:
            if (intf->local_oam_mode == OAM_MODE_PASSIVE) {
                OAM_LOG_DEBUG("Unexpected VarRsp\n");
                return -1;
            }

            while (curr) {
                if (oam_proc_var_cont((oam_var_cont_t *)curr, &max, &next_var_cont)) {
                    OAM_LOG_DEBUG("Invalid variable container\n");
                    return -1;
                }

                curr = (oam_uint8 *)next_var_cont;
            }
        ...
        case OAM_PDU_CODE_LOOPBACK:
            if (!(intf->remote_oam_config & OAM_INFO_CONFIG_OAM_MODE)) {
                OAM_LOG_DEBUG("Unexpected loopback control\n");
                return -1;
            }

            if ((curr[0] != OAM_LOOPBACK_COMMAND_ENABLE) &&
                (curr[0] != OAM_LOOPBACK_COMMAND_DISABLE)) {
                OAM_LOG_DEBUG("Unknown remote loopback command\n");
            }
        ...
        default:
            OAM_LOG_DEBUG("Unknown OAMPDU code\n");
            return -1;
    }
...
/*****************************************************************************/
{
    ...
    oam_int16 tmp;
    oam_uint8 *pos;
    oam_uint32 vendor_len;

    len = sizeof(oam_pdu_hdr_t);
    pos = (oam_uint8 *)pdu + len;
    ...
    if (intf->local_pdu != OAM_LOCAL_PDU_LF_INFO) {
...
#ifdef HAVE_DPOE_OAM
        pdu->local_info.oam_config = 0x14;
#endif
        pdu->local_info.oampdu_config = ntohs(intf->remote_tlv_info.oampdu_config);
        if ((pdu->local_info.oampdu_config != intf->local_oampdu_config) &&
            (pdu->local_info.oampdu_config <= OAM_MAX_OAMPDU_SIZE)) {
...
#ifdef HAVE_DPOE_OAM
        memcpy(pdu->local_info.oui, oam_oui_tk, OAM_OUI_LEN); /* for sumitomo OLT */
#endif
        ...
        if (intf->d2_compatible) {
            ...
            /* interop */
            pdu->local_info.vendor[0] = 0x20;
            pdu->local_info.vendor[1] = 0x00;
            pdu->local_info.vendor[2] = 0x00;
            pdu->local_info.vendor[3] = 0x01;
        }

        /* Local_info Vendor build */
        oam_vendor_build_handler(OAM_VENDOR_HANDLE_INFO_BUILD,
                                 intf->llid, (oam_uint8 *) &pdu->local_info, sizeof(pdu->local_info));
        ...
        vendor_len = oam_vendor_build_handler(OAM_VENDOR_HANDLE_EXT_INFO_BUILD,
                                              intf->llid, pos, OAM_MAX_OAMPDU_SIZE - len);
...
/* CHIP       : ----                                                         */
/*                                                                           */
static oam_int32 oam_build_pdu_var_rsp(
    oam_if_t *intf,
...
/*****************************************************************************/
{
    ...
    if (intf->d2_compatible) {
        ...
        while (pParm_d2->branch) {
            ...
            size = oam_d2_set_mib_variable(intf, pParm_d2->branch, pParm_d2->leaf, pD2Cont,
                                           tmpl);
            totalSize += size;
            pData += size;
            pD2Cont = (oam_d2_var_cont_t *)pData;
        ...
        *pData = 0;
        ...
        OAM_LOG_DEBUG( "sending D2 oam len %d\n", totalSize);
        intf->var_rsp_tx++;
        return totalSize;
    }
    ...
    while (pParm->branch) {
        ...
        if (pParm->width == sizeof(unsigned long)) {
            ...
            tmpl = ntohl(tmpl);
            size = oam_set_mib_variable(intf, pParm->branch, ntohs(pParm->leaf), pCont,
                                        tmpl);
        }
        else if (pParm->width == sizeof(oam_int16)) {
...
/*****************************************************************************/
{
    ...
    oam_uint8 reply = OAM_FALSE;
    oam_int16 flags = ntohs(pdu->hdr.flags);
    ...
    if (flags & OAM_FLAG_LINK_FAULT || flags & OAM_FLAG_DYING_GASP) {
        ...
        OAM_LOG_DEBUG( "Link fault/Dying gasp event\n");
        return;
    }
#if 0
    ...
    if (intf->lb_state_peer == OAM_LB_STATE_PEER_INIT) {
        if ((!intf->d2_compatible &&
            ...
            /*oam_env_loopback_ind(intf->llid, OAM_TRUE);*/
        }
    }
        ...
        if ((!intf->d2_compatible &&
            ...
            /* oam_env_loopback_ind(intf->llid, OAM_FALSE);*/
        }
    }

    OAM_LOG_DEBUG("req oam ctl for llid %d\r\n", intf->llid);
    oam_req_oam_ctl(intf, &parm);
    ...
    //if (reply) {
        OAM_LOG_DEBUG("OAM reply for llid %d\r\n", intf->llid);
...
/*****************************************************************************/
{
    ...
    if (events) {
        /* oam_env_critical_event_ind(intf->llid, events); */
    }
}
...
/*****************************************************************************/
{
    ...
    /*
       ...
       while (curr) {
       oam_proc_tlv((oam_uint8 *)curr, &max, &next);
       if (curr->type != 0) {
       num++;
       }
       ...
       oam_env_event_ind(intf->llid, num, &pdu->event[0]);
       */
}
/*****************************************************************************/
...
/*****************************************************************************/
{
    if (oam_proc_pdu_sanity(intf, pdu, len)) {
        OAM_LOG_DEBUG( "OAMPDU sanity check fails\n");
        //return;
    }
    ...
    switch (pdu->code) {
        case OAM_PDU_CODE_INFO:
            OAM_LOG_DEBUG( "OAMPDU info rx from llid %d\n", intf->llid);
            oam_proc_pdu_info(intf, (oam_pdu_info_t *)pdu, len);
            break;

        case OAM_PDU_CODE_EVENT:
            OAM_LOG_DEBUG( "OAMPDU event rx from llid %d\n", intf->llid);
            oam_proc_pdu_event(intf, (oam_pdu_event_t *)pdu, len);
            break;

        case OAM_PDU_CODE_VAR_REQ:
            OAM_LOG_DEBUG("OAMPDU varreq rx from llid %d\n", intf->llid);
        ...
        case OAM_PDU_CODE_VAR_RSP:
            OAM_LOG_DEBUG("OAMPDU varrsp rx from llid %d\n", intf->llid);
            /* oam_proc_pdu_var_rsp(intf, (oam_pdu_var_rsp_t *)pdu, len); */
            break;

        case OAM_PDU_CODE_LOOPBACK:
            OAM_LOG_DEBUG("OAMPDU loopback rx from llid %d\n", intf->llid);
            oam_proc_pdu_loopback(intf, (oam_pdu_loopback_t *)pdu, len);
            break;

        case OAM_PDU_CODE_ORG_SPEC:
            OAM_LOG_DEBUG("OAMPDU orgspec rx from llid %d\n", intf->llid);
        ...
        default:
            intf->unsupported_rx++;
            oam_printf("Unknown OAMPDU code %d rx from llid %d\n", pdu->code, intf->llid);
            break;
    }
}
...
void oam_llid_registration(oam_llid_t llid, oam_uint16 index)
/*                                                                           */
...
/*****************************************************************************/
{
    ...
    if (!oam_intf) {
        OAM_LOG_MAJ("oam_intf_alloc: failed (1)\n");
        return;
    }

    oam_intf_init(oam_intf, llid, OAM_MODE_PASSIVE);

    oam_llid_mac_get_adapt(index, oam_intf->macaddr);

    oam_intf->index = index;
...
/* CHIP       : ----                                                         */
/*                                                                           */
void oam_llid_deregistration(oam_llid_t llid)
/*                                                                           */
/* INPUTS  : o llid - onu llid                                               */
/* OUTPUTS : N/A                                                             */
/* DESCRIPTION:                                                              */
...
/*****************************************************************************/
{
    ...
    if (oam_intf != NULL) {
        oam_intf_free(oam_intf);
        oam_pon_mac_reset_adapt();
    }
    else {
        OAM_LOG_MAJ("oam_intf_find: failed\n");
    }
}
